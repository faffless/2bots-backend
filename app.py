...
SESSION_LAST_ACTIVE: Dict[str, float] = {}  # sid -> timestamp
...
async def _stream_batch(engine: TwoBotsEngine, batch: list, who_generated: str, sid: str,
                        prefetched_audio: list = None):
    """Stream a pre-generated autopilot batch: uses prefetched TTS if available,
    otherwise generates ALL TTS first. Then streams text+audio pairs with zero gap.
    ...
    # Use prefetched TTS if available, otherwise generate fresh
    if prefetched_audio and len(prefetched_audio) == len(batch):
        batch_audio = prefetched_audio
        log("autopilot", f"Using prefetched TTS for {len(batch)} msgs")
    else:
        gpt_voice = engine.get_gpt_voice()
        claude_voice = engine.get_claude_voice()
    ...
    async def _do_prefetch():
        try:
            ...
            log("prefetch", f"Prefetch ready (text+TTS) for {sid[:8]}... ({len(pf_batch)} msgs via {next_generator})")
        ...
        if is_pingpong:
            ...
            async def _prefetch_batch():
                try:
                    ...
                    log("prefetch", f"Start prefetch ready (text+TTS) for {sid[:8]}... ({len(pf_batch)} msgs via {next_generator})")
                except Exception as e:
...
async def autopilot_stream(request: Request, req: AutopilotRequest):
    ...
    async def generate():
        async for event in _stream_batch(engine, batch, req.who_generates, sid, prefetched_audio=cached_audio):
            yield event
        save_messages_only(sid, engine)
